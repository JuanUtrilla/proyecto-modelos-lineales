...
def atipicosAmissing(varaux):
    """
    Identifica valores atípicos en una serie y los reemplaza por NaN.
    
    Devuelve:
    - Serie con atípicos reemplazados por NaN
    - Número de atípicos
    - Serie con los valores que han sido reemplazados (índice + valor)
    """
    ...
    mask = criterio1 & criterio2

    # valores que se van a modificar
    valores_atipicos = varaux[mask]

    # reemplazo
    var = varaux.copy()
    var[mask] = np.nan

    return [var, mask.sum(), valores_atipicos]


def patron_perdidos(datos_input, threshold=0.05):
    """
    Visualiza la correlación de patrones de valores ausentes
    solo para variables con una proporción relevante de missing.
    """

    # Selección de variables con missing relevante
    cols_miss = datos_input.columns[
        datos_input.isna().mean() > threshold
    ]

    if len(cols_miss) < 2:
        print("No hay suficientes variables con missing para analizar correlación.")
        return

    # Matriz de correlación de valores ausentes
    correlation_matrix = datos_input[cols_miss].isna().corr()
